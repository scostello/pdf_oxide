...
def test_adaptive_heuristics():
    ...
    try:
        ...
        print(f"   ℹ️  Tagged PDF - uses structure tree (Phase 1)")
        print(f"\n   First 200 chars:")
    ...
    try:
        ...
        markdown = doc.to_markdown(0)
        print(f"   ✅ Converted to markdown ({len(markdown)} chars)")
        print(f"   ℹ️  Untagged PDF - uses adaptive XY-Cut (Phase 2)")
        print(f"\n   First 200 chars:")
        print(f"   {markdown[:200]!r}")
    except Exception as e:
        print(f"   ❌ Error: {e}")
    ...
    try:
        ...
        print(f"   ℹ️  Large document - adaptive params computed per page")
